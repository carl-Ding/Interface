...
from Interface_Python_autoFramework.public.get_email_parameter import GetEmailParameter
...
def run_case(all_case, reportName='report'):
    ...
    report_abspath = os.path.join(report_path, now + 'xxx项目Interface自动化测试报告.html')
    with open(report_abspath, 'wb') as fp:
        runner = HTMLTestRunner.HTMLTestRunner(stream=fp, title='xxx项目Interface自动化测试报告', description='用例执行情况')
        runner.run(all_case)
